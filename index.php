...
function autoLocale()
{
    ...
    setlocale(LC_TIME,'fr_FR','fra');  // LC_TIME = Set local for date/time format only.                              //AlbinoMouse - locale fixée en français
}
...
function showDaily()
{
    ...
    */
    ...
    $PAGE->assign('day',utf8_encode(strftime('%A %d %B %Y',linkdate2timestamp($day.'_000000'))));             //AlbinoMouse - suppression de la virgule dans la date
...
function renderPage()
{
    ...
    // -------- About page
    if (isset($_SERVER["QUERY_STRING"]) && startswith($_SERVER["QUERY_STRING"],'do=about'))
    {
        $PAGE = new pageBuilder;
        $PAGE->assign('linkcount',count($LINKSDB));
        $PAGE->renderPage('about');
        exit;
    }
    ...
    // -------- Budget page
    if (isset($_SERVER["QUERY_STRING"]) && startswith($_SERVER["QUERY_STRING"],'do=budget'))
    {
    ...
    if (isset($_SERVER["QUERY_STRING"]) && startswith($_SERVER["QUERY_STRING"],'do=tagcloud'))
    {
        ...
        {
            $tagList[$key] = array('count'=>$value,'size'=>max(150*$value/$maxcount,8));
        }
        $PAGE = new pageBuilder;
        $PAGE->assign('linkcount',count($LINKSDB));
    ...
    if (isset($_SERVER["QUERY_STRING"]) && startswith($_SERVER["QUERY_STRING"],'do=database'))
    {
        if (!empty($_POST['mysql_login']) && !empty($_POST['mysql_host']) && !empty($_POST['mysql_db'])) {
            $GLOBALS['mysql_login'] = $_POST['mysql_login'];
            $GLOBALS['mysql_db'] = $_POST['mysql_db'];
            $GLOBALS['mysql_host'] = $_POST['mysql_host'];
            if(!empty($_POST['mysql_pass'])) {
                $GLOBALS['mysql_pass'] = $_POST['mysql_pass'];
            }
            writeConfig();
            echo '<script language="JavaScript">alert("Configuration was saved.");document.location=\'?do=tools\';</script>';
            exit;
            ...
            $PAGE->assign('linkcount',count($LINKSDB));
            $PAGE->assign('token',getToken());
            $PAGE->assign('mysql_host',htmlspecialchars( empty($GLOBALS['mysql_host']) ? '' : $GLOBALS['mysql_host'] , ENT_QUOTES));
            $PAGE->assign('mysql_login',htmlspecialchars( empty($GLOBALS['mysql_login']) ? '' : $GLOBALS['mysql_login'] , ENT_QUOTES));
            $PAGE->assign('mysql_db',htmlspecialchars( empty($GLOBALS['mysql_db']) ? '' : $GLOBALS['mysql_db'] , ENT_QUOTES));
            $PAGE->assign('title',htmlspecialchars( empty($GLOBALS['title']) ? '' : $GLOBALS['title'] , ENT_QUOTES));
            $PAGE->renderPage('database');
            exit;
        }
    }
    ...
    if (isset($_GET['post']))
    {
        ...
        $i=strpos($url,'?fb_'); if ($i!==false) $url=substr($url,0,$i);              			//AlbinoMouse - nettoyage des url facebook
...
function install()
{
    ...
    if (!empty($_POST['setlogin']) && !empty($_POST['setpassword']) && !empty($_POST['setmysqlhost']) && !empty($_POST['setmysqllogin']) && !empty($_POST['setmysqlpassword']) && !empty($_POST['setmysqldb']))
    {
        $tz = 'UTC';
        if (!empty($_POST['continent']) && !empty($_POST['city']))
            if (isTZvalid($_POST['continent'],$_POST['city']))
        ...
        $GLOBALS['api_key'] = sha1(uniqid('',true).'_'.mt_rand());
        $GLOBALS['api_hash'] = substr(sha1(uniqid('',true).'_'.mt_rand()), 0, 16);
        $GLOBALS['mysql_host'] = $_POST['setmysqlhost'];
        $GLOBALS['mysql_login'] = $_POST['setmysqllogin'];
        $GLOBALS['mysql_pass'] = $_POST['setmysqlpassword'];
        $GLOBALS['mysql_db'] = $_POST['setmysqldb'];
        writeConfig();
...
function writeConfig()
{
    ...
    $config .= '$GLOBALS[\'mysql_login\']='.var_export($GLOBALS['mysql_login'],true).';';
    $config .= '$GLOBALS[\'mysql_pass\']='.var_export($GLOBALS['mysql_pass'],true).';';
    $config .= '$GLOBALS[\'mysql_host\']='.var_export($GLOBALS['mysql_host'],true).';';
    $config .= '$GLOBALS[\'mysql_db\']='.var_export($GLOBALS['mysql_db'],true).';';
    $config .= ' ?>';
